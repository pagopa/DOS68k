...
def test_jwt_check_valid_bearer_token():
    ...
    result = AuthService(auth_interface=MockAuthProvider()).jwt_check(
        authorization="Bearer valid-token"
    )

    assert result["sub"] == MOCK_JWT_PAYLOAD["sub"]
    assert result["role"] == MOCK_JWT_PAYLOAD["role"]
...
def test_jwt_check_no_bearer_prefix():
    ...
    with pytest.raises(HTTPException) as exc_info:
        AuthService(auth_interface=MockAuthProvider()).jwt_check(
...
def test_jwt_check_expired_token():
    ...
    with pytest.raises(HTTPException) as exc_info:
        AuthService(auth_interface=ExpiredProvider()).jwt_check(
...
def test_jwt_check_invalid_key():
    ...
    with pytest.raises(HTTPException) as exc_info:
        AuthService(auth_interface=InvalidKeyProvider()).jwt_check(
...
def test_jwt_check_any_exception_becomes_401():
    ...
    with pytest.raises(HTTPException) as exc_info:
        AuthService(auth_interface=BrokenProvider()).jwt_check(
            authorization="Bearer some-token"
        )
