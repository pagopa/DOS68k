...
class Message(BaseModel):
    ...
    model_config: ConfigDict = ConfigDict(validate_by_alias=True, populate_by_name=True)

    index_id: Annotated[
        str, Field(description="Vector db index to add the document", alias="indexId")
    ]
    user_id: Annotated[str, Field(description="User id", alias="userId")]
    object_key: Annotated[
        str,
        Field(
            description="Key of the document to store on the vector db",
            alias="objectKey",
        ),
    ]
    document_type: Annotated[
        ...
        Field(
            description="Type of document to store on the vector db",
            alias="documentType",
        ),
    ]
