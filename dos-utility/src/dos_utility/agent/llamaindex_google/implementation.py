...
from .env import LlamaIndexGoogleAgentSettings, get_llamaindex_google_agent_settings
...
class LlamaIndexGoogleAgent(AgentInterface):
    """`AgentInterface` implementation backed by LlamaIndex ReActAgent + Google GenAI.

    The whole LlamaIndex / Google GenAI surface is contained in this provider:
    ...
    def __init__(
        ...
        *,
        ...
        try:
            self._settings: LlamaIndexGoogleAgentSettings = (
                get_llamaindex_google_agent_settings()
            )
            ...
            self._embed_model: BaseEmbedding = get_embed_model(
                ...
                task_type=self._settings.EMBED_TASK,
                retries=self._settings.EMBED_RETRIES,
                retry_min_seconds=self._settings.EMBED_RETRY_MIN_SECONDS,
            )
            ...
            logger.debug(
                "LlamaIndexGoogleAgent initialized - model_id=%s, tools=%s",
    ...
    async def is_healthy(self: Self) -> bool:
        try:
            ...
            logger.warning("LlamaIndexGoogleAgent health check failed: %s", e)
            return False

    async def close(self: Self) -> None:
        # The Google GenAI client does not currently require explicit cleanup.
        return None
...
def get_llamaindex_google_agent(
    *,
    rag_tools: List[RagToolSpec],
    agent_config: AgentConfig,
    output_schema: Optional[Type[BaseModel]] = None,
) -> LlamaIndexGoogleAgent:
    return LlamaIndexGoogleAgent(
        rag_tools=rag_tools,
