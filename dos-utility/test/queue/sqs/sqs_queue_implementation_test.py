...
from dos_utility.queue.sqs import implementation, SQSQueue, get_sqs_queue
...
def test_instantiate_sqs_queue(monkeypatch: pytest.MonkeyPatch):
    ...
    monkeypatch.setattr(
        implementation, "get_sqs_queue_settings", get_sqs_queue_settings_mock
    )
    monkeypatch.setattr(
        implementation,
...
async def test_sqs_dequeue_new_message(monkeypatch: pytest.MonkeyPatch):
    ...
    monkeypatch.setattr(
        implementation.boto3, "client", boto3_client_new_message_dequeue_mock
    )
    monkeypatch.setattr(
        implementation, "get_sqs_queue_settings", get_sqs_queue_settings_mock
    )
