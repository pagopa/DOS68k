...
async def test_create_index_failed(monkeypatch: pytest.MonkeyPatch):
    ...
    monkeypatch.setattr(
        implementation, "AsyncQdrantClient", AsyncQdrantClientCreateCollectionFailedMock
    )
    ...
    async with QdrantVectorDB() as db:
        with pytest.raises(
            expected_exception=IndexCreationException,
            match=f"Qdrant gave a negative output when creating index '{index_name}'",
        ):
...
async def test_delete_index_failed(monkeypatch: pytest.MonkeyPatch):
    ...
    monkeypatch.setattr(
        implementation, "AsyncQdrantClient", AsyncQdrantClientDeleteCollectionFailedMock
    )
    ...
    async with QdrantVectorDB() as db:
        with pytest.raises(
            ...
            match=f"Qdrant gave a negative output when deleting index '{index_name}'",
        ):
...
async def test_put_objects_upsert_failed(monkeypatch: pytest.MonkeyPatch):
    ...
    monkeypatch.setattr(
        implementation, "AsyncQdrantClient", AsyncQdrantClientPutObjectsFailedMock
    )
    ...
    async with QdrantVectorDB() as db:
        with pytest.raises(
            expected_exception=PutObjectsException,
            match=f"Adding objects to index '{index_name}' did not complete successfully.",
        ):
...
async def test_delete_objects_failed(monkeypatch: pytest.MonkeyPatch):
    ...
    monkeypatch.setattr(
        implementation, "AsyncQdrantClient", AsyncQdrantClientDeleteObjectsFailedMock
    )
    ...
    async with QdrantVectorDB() as db:
        with pytest.raises(
            expected_exception=DeleteObjectsException,
            match=f"Deleting objects from index '{index_name}' did not complete successfully.",
        ):
...
async def test_semantic_search(monkeypatch: pytest.MonkeyPatch):
    ...
    assert all(
        result.score >= score_threshold and result.score <= 1.0 for result in results
    )
    ...
    async with QdrantVectorDB() as db:
        results = await db.filter_search(
            ...
            filters=MetadataFilters(
                filters=[
                    MetadataFilter(
                        key="filename", value="file1.txt", operator=FilterOperator.ANY
                    )
                ]
            ),
            max_results=5,
    ...
    async with QdrantVectorDB() as db:
        results = await db.filter_search(
            ...
            filters=MetadataFilters(
                filters=[
                    MetadataFilter(
                        key="content", value="", operator=FilterOperator.IS_EMPTY
                    )
                ]
            ),
            max_results=5,
