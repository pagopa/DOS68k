...
class AWSClientMock:
    ...
    def put_object(
        self: Self, Bucket: str, Key: str, Body: BytesIO, ContentType: str
    ) -> None:
        pass
...
def get_aws_storage_settings_mock() -> AWSStorageSettingsMock:
    return AWSStorageSettingsMock(S3_ENDPOINT=None, S3_REGION="us-east-1")


def get_aws_storage_settings_with_endpoint_mock() -> AWSStorageSettingsMock:
    return AWSStorageSettingsMock(
        S3_ENDPOINT="http://localhost:4566", S3_REGION="us-east-1"
    )
...
def aws_client_list_buckets_exception_mock(
    *args, **kwargs
) -> AWSClientListBucketsExceptionMock:
