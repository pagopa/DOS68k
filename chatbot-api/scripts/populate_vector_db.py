...
TOPICS: dict[str, list[dict]] = {
    ...
    # NOTE: invented sample data for local testing of the PagoPA configuration.
    # The index name MUST match the `index_id` in
    # src/modules/chatbot/tool/config/devportal.yaml (devportal-index).
    "devportal-index": [
        {
            "filename": "app-io.md",
            "chunk_id": 0,
            "content": (
                "App IO is the mobile access point for the public services of Italian public "
                "administrations, available on iOS and Android. Institutions (enti) integrate "
                "with App IO through the IO Backend REST API. Onboarding requires registering the "
                "organisation on the developer.pagopa.it self-care area, obtaining a primary and "
                "secondary API Key, and configuring at least one service. Authentication to the "
                "API is performed via the 'Ocp-Apim-Subscription-Key' HTTP header carrying the "
                "API Key. The base URL for production is https://api.io.pagopa.it/api/v1 and the "
                "test environment is https://api.io.italia.it/api/v1."
            ),
        },
        {
            "filename": "app-io.md",
            "chunk_id": 1,
            "content": (
                "To send a message through App IO, an institution calls POST /messages with a "
                "JSON body containing the fiscal_code of the recipient, a subject (10-120 chars), "
                "and a markdown body (80-10000 chars). The recipient must have an active profile "
                "and have given consent to the sending service. Messages can optionally carry a "
                "payment_data object to attach a pagoPA payment notice, and a due_date for "
                "reminders. The API returns 201 Created with an id; delivery and read status are "
                "retrieved via GET /messages/{fiscal_code}/{id}. Rate limits apply per API Key: "
                "the default quota is 1000 messages per minute in production."
            ),
        },
        {
            ...
            "chunk_id": 0,
            "content": (
                "Firma con IO lets institutions collect Qualified Electronic Signatures (QES) "
                "from citizens directly inside App IO. Technically, the institution creates a "
                "'signature request' (richiesta di firma) via the Firma con IO API by uploading "
                "one or more PDF documents and defining the signature fields with their page and "
                "coordinates. Each signature is a one-shot QES generated by a Qualified Trust "
                "Service Provider (QTSP). Integration uses the same self-care API Key model as "
                "App IO. The dossier (fascicolo) groups documents that the citizen signs in a "
                "single session."
            ),
        },
        {
            ...
            "chunk_id": 1,
            "content": (
                "The Firma con IO flow is: (1) POST /dossiers to create a document set, (2) POST "
                "/signature-requests referencing the dossier and the target fiscal_code, (3) the "
                "citizen receives a message in App IO and authorises the signature with SPID/CIE "
                "level 2 plus a one-time code, (4) the institution polls GET /signature-requests/"
                "{id} until status becomes SIGNED, (5) the signed PAdES document is downloaded "
                "via GET /signature-requests/{id}/documents/{doc_id}. Webhooks can be configured "
                "to receive status callbacks instead of polling. Supported input format is "
                "PDF/A; the output is a PAdES-LTA signed PDF with embedded timestamp."
            ),
        },
        {
            ...
            "chunk_id": 0,
            "content": (
                "PDND Interoperabilita (Piattaforma Digitale Nazionale Dati) enables secure "
                "machine-to-machine data exchange between public and private organisations. A "
                "provider (erogatore) publishes an e-service describing its API; a consumer "
                "(fruitore) subscribes by requesting access and accepting the terms. Access is "
                "governed by voucher tokens: the consumer generates a signed client assertion "
                "(JWT) using a registered key pair and exchanges it at the PDND authorization "
                "server for a short-lived voucher (JWT, valid 600 seconds) following the OAuth "
                "2.0 client credentials flow with private_key_jwt client authentication."
            ),
        },
        {
            "filename": "pdnd-interoperabilita.md",
            "chunk_id": 1,
            "content": (
                "To call a PDND e-service, the consumer sends the voucher in the 'Authorization: "
                "Bearer <voucher>' header to the provider endpoint. Each request and the client "
                "assertion are validated against the keys registered in the PDND key registry. "
                "The client assertion JWT must include the iss, sub (client id), aud "
                "(authorization server), jti, and a purposeId claim identifying the declared "
                "finality of the data access. Test and production environments are separated; "
                "the test authorization server is https://auth.uat.interop.pagopa.it and "
                "production is https://auth.interop.pagopa.it."
            ),
        },
        {
            "filename": "send.md",
            "chunk_id": 0,
            "content": (
                "SEND (Servizio Notifiche Digitali, formerly Piattaforma Notifiche - SEND) is the "
                "platform that digitalises the sending of legal notifications (notifiche a valore "
                "legale) by public administrations. The sending institution creates a notification "
                "via the b2b REST API by submitting recipient data, the notified documents "
                "(uploaded to a presigned S3 URL), and the payment information. SEND attempts "
                "digital delivery first (App IO, PEC, email/SMS courtesy) and falls back to "
                "physical registered mail (raccomandata) when no digital domicile is available."
            ),
        },
        {
            "filename": "send.md",
            "chunk_id": 1,
            "content": (
                "The SEND integration flow: (1) PRE-LOAD documents with POST /delivery/v2.3/"
                "requests obtaining presigned upload URLs, (2) upload each file with the returned "
                "secret and SHA-256 checksum, (3) submit the notification with the document keys "
                "and recipients, (4) track legal status transitions (ACCEPTED, DELIVERING, "
                "VIEWED, EFFECTIVE_DATE) via GET /requests/{iun} or via the timeline webhook. "
                "Each notification is identified by a IUN (Identificativo Univoco Notifica). "
                "Authentication uses the 'x-api-key' header issued from the SEND self-care area."
            ),
        },
        {
            "filename": "pari.md",
            "chunk_id": 0,
            "content": (
                "PARI (Piattaforma di Accreditamento e Rendicontazione Integrata) is the PagoPA "
                "component used by institutions to manage accreditation and reconciliation of "
                "collected payments. Through the PARI API an Ente Creditore retrieves the daily "
                "settlement flows (flussi di rendicontazione), reconciles each pagoPA receipt "
                "(Ricevuta Telematica, RT) against the corresponding bank transfer, and exports "
                "accounting reports. The flows follow the standard pagoPA flusso di rendicontazione "
                "XML schema and are downloaded by date range via GET /flows."
            ),
        },
        {
            "filename": "pago-pa.md",
            ...
            "content": (
                ...
                "administration. An Ente Creditore registers and exposes a service to "
                "generate payment notices (avvisi di pagamento) identified by a IUV "
                "(Identificativo Univoco Versamento) combined with the creditor's fiscal code to "
                "form the Numero Avviso. Technical integration uses the ACA (Archivio Centralizzato "
                "Avvisi) REST API to create, update, and delete debt positions. Each notice "
                "carries an amount, due date, payee, and a causale describing the payment reason."
            ),
        },
        {
            "filename": "pago-pa.md",
            "chunk_id": 1,
            "content": (
                "Citizens pay pagoPA notices through Checkout, the PagoPA payment web flow, or "
                "inside partner apps via the eCommerce API. The eCommerce flow is: (1) verify the "
                "notice with GET /payment-requests/{rptId}, (2) activate the payment to lock the "
                "amount, (3) create a transaction and choose a payment method (card, bank account, "
                "or alternative method), (4) authorise via the selected PSP, (5) receive the "
                "outcome and the RT. The rptId concatenates the creditor fiscal code and the "
                "Numero Avviso. PSPs (Prestatori di Servizi di Pagamento) integrate through the "
                "Nodo dei Pagamenti SPC using the SOAP/REST primitives of the pagoPA SANP spec."
            ),
        },
        {
            "filename": "srtp.md",
            "chunk_id": 0,
            "content": (
                "SRTP (SEPA Request-to-Pay) is the messaging service that lets a payee send a "
                "structured request to pay to a payer's bank or app, decoupled from the actual "
                "payment execution. In the PagoPA context, SRTP is used to push pagoPA payment "
                "notices to citizens' banking apps as a Request-to-Pay message compliant with the "
                "EPC SRTP scheme. The institution submits an SRTP message referencing the pagoPA "
                "IUV; the payer's bank presents it and, on acceptance, triggers a SEPA Credit "
                "Transfer or instant payment that settles the notice."
            ),
        },
        {
            "filename": "srtp.md",
            "chunk_id": 1,
            "content": (
                "SRTP messages follow the ISO 20022 pain.013 (CreditorPaymentActivationRequest) "
                "and pain.014 (status response) standards. The PagoPA SRTP API accepts an "
                "activation request containing the expiry date, the amount, the creditor and "
                "debtor identifiers, and the pagoPA payment reference. Status callbacks report "
                "ACCEPTED, REJECTED, or EXPIRED. Authentication relies on mutual TLS plus the "
                "self-care API Key. The service is exposed under the developer.pagopa.it SRTP "
                "section together with the technical reference for the message validation rules."
            ),
        },
        {
            "filename": "api.md",
            "chunk_id": 0,
            "content": (
                "All PagoPA Developer Portal products expose their API specifications as OpenAPI "
                "3.0 documents downloadable from developer.pagopa.it. Authentication is API-Key "
                "based for institution-facing APIs (the key is sent in the 'Ocp-Apim-Subscription-"
                "Key' or 'x-api-key' header depending on the product) and OAuth 2.0 voucher based "
                "for PDND interoperability. API Keys are managed in the self-care (area riservata) "
                "console, where each product subscription provides a primary and a secondary key "
                "to allow zero-downtime key rotation. Test (UAT/collaudo) and production "
                "environments use distinct base URLs and distinct keys."
            ),
        },
        {
            "filename": "api.md",
            "chunk_id": 1,
            "content": (
                "Common HTTP conventions across PagoPA APIs: requests and responses use JSON "
                "(application/json) unless a binary upload is involved; errors follow RFC 7807 "
                "problem+json with a 'status', 'title', and 'detail'; 401 indicates a missing or "
                "invalid API Key, 403 a key without the required subscription, 429 a rate-limit "
                "breach (retry after the Retry-After header), and 5xx a transient platform error "
                "that should be retried with exponential backoff. Idempotency on creation "
                "endpoints is achieved by supplying a client-generated unique identifier (IUV, "
                "IUN, or dossier id) so retries do not create duplicates."
            ),
        },
        {
            ...
            "chunk_id": 0,
            "content": (
                "The PagoPA Developer Portal periodically hosts technical webinars for developers "
                "and institutions covering onboarding, API integration, and migration topics. "
                "Recordings and slide decks are published in the Webinars section of "
                "developer.pagopa.it. Recurring sessions include 'Integrare pagoPA con la ACA "
                "API', 'Onboarding tecnico su SEND', and 'Autenticazione e voucher su PDND'. "
                "Registration is free; each webinar provides a Q&A transcript and links to the "
                "relevant OpenAPI specifications and quick-start guides referenced during the "
                "session."
            ),
        },
    ],
